...
class _DenseLayer(nn.Module):
    def __init__(self, num_input_features, growth_rate, bottleneck_width, drop_rate):
        ...
        self.branch2c = BasicConv2d(inter_channel, growth_rate, kernel_size=1)

    def forward(self, x):
        branch1 = self.branch1a(x)
        branch1 = self.branch1b(branch1)
        branch1 = self.branch1c(branch1)
...
def jw_pellelite4():
    model = PeleeNet(growth_rate=8, num_init_features=12, block_config=[3, 5, 8, 4], bottleneck_width=[2, 4, 4, 8])
    return model
